...
int			pgqp_storage_memory;	/* memory used to store plan and query
									 * texts */
...
bool		pgqp_normalize_plans;	/* Normalize plans so pans differ in
									 * constans have the same planId */
int			example_plan_format = EXPLAIN_FORMAT_JSON;	/* Plan representation
														 * style */
 ...
 */
static PlannedStmt *
pgqp_planner(Query *parse, const char *query_string,
			 int cursorOptions, ParamListInfo boundParams
#if PG_VERSION_NUM >= 190000
			 ,ExplainState *es
#endif
...
pgqp_ExecutorEnd(QueryDesc *queryDesc)
{
		...
		pgqp_enabled(pgqp_exec_nested_level))
	{
		 ...
		 * changing execution plan - it could be changing statistics. Cost,
		 * the number of rows,width and other params  will be stored in
		 * additional fields.
		 */
 ...
 */
Datum
pg_stat_query_plans_reset_queryid(PG_FUNCTION_ARGS)
{
	Oid			userid;
	Oid			dbid;
	uint64		queryid;
 ...
 */
Datum
pg_stat_query_plans_reset_minmax(PG_FUNCTION_ARGS)
{
...
static int
pgqp_add_counters_data(pgqpCounters * c, Datum values[0],
					   pgqpStoreKind start_kind)
{
	int			i = 0;
	double		stddev;
 ...
 */
Datum
pg_stat_query_plans_plans_1_0(PG_FUNCTION_ARGS)
{
...
static void
pg_stat_query_plans_plan_internal(FunctionCallInfo fcinfo,
								  pgqpVersion api_version,
								  bool showtext)
{
 ...
 */
Datum
pg_stat_query_plans_sql_1_0(PG_FUNCTION_ARGS)
{
	bool		showtext = PG_GETARG_BOOL(0);
...
static void
pg_stat_query_plans_stat_internal(FunctionCallInfo fcinfo,
								  pgqpVersion api_version,
								  bool showtext)
{
	ReturnSetInfo *rsinfo = (ReturnSetInfo *) fcinfo->resultinfo;
 ...
 */
Datum
pg_stat_query_plans_info(PG_FUNCTION_ARGS)
{
	TupleDesc	tupdesc;
