 ...
 */
char *
get_decoded_text(volatile pgqpTextStorageEntry * s)
{
...
pgqp_store_text(const char *data, pgqpTextsKind kind,
				uint64 id_low, uint64 id_high)
{
	...
	if (!found)
	{
		...
		if (needed_encoding == PGQP_PGLZ)
		{
			...
			{
				/*
				 * most probably data is not compressable - so store them in
				 * plain text
				 */
				needed_encoding = PGQP_PLAINTEXT;
			}
...
void
pgqp_remove_text(pgqpTextStorageEntry * entry)
{
	/* Safety check... */
	if (!pgqp || !pgqp_texts || !pgqp_storage)
		return;
 ...
 */
static void
pgqp_update_counters(volatile pgqpCounters * counters,
					 pgqpStoreKind kind, double total_time,
					 uint64 rows, const BufferUsage *bufusage,
#if PG_VERSION_NUM >= 130000
					 const WalUsage *walusage,
 ...
 */
static uint64
pgqp_hash_string(const char *str, int len)
{
 ...
 */
void
pgqp_store(const char *query, uint64 planId, uint64 queryId,
...
)
{
	...
	if (planId != invalid_id)
	{
		...
		if (!plan_entry)
		{
			...
			/*
			 * Query entry could be wiped out here so we should search it
			 * again
			 */
			entry = (pgqpEntry *) hash_search(pgqp_queries, &key, HASH_FIND, NULL);
			if (entry)
 ...
 */
pgqpEntry *
pgqp_query_alloc(pgqpQueryHashKey * key, bool sticky,
				 int64 generation)
{
	pgqpEntry  *entry;
	bool		found;
 ...
 */
pgqpPlanEntry *
pgqp_plan_alloc(pgqpPlanHashKey * key, bool sticky,
				int64 generation)
{
	pgqpPlanEntry *entry;
	bool		found;
 ...
 */
static int
entry_dealloc_cmp(const void *lhs, const void *rhs)
{
 ...
 */
static int
plan_entry_dealloc_cmp(const void *lhs, const void *rhs)
{
 ...
 */
static int
entry_gc_cmp(const void *lhs, const void *rhs)
{
...
pgqp_gc_storage(void)
{
	...
	/*
	 * Sort entries by offset and deallocate not used entries. Copy string
	 * data in a shared memory storage while deallocating unused items.
	 */
	i = 0;
 ...
 */
bool
pgqp_need_gc(bool already_started, int64 queries_size, int64 plans_size)
{
 ...
 */
static bool
pgqp_need_gc_stat(int64 queries_size)
{
...
pgqp_dealloc(void)
{
	...
	/*
	 * Sort entries by usage and deallocate while not satisfied pgqp_need_gc
	 * criteria. While we're scanning the table, apply the decay factor to the
	 * usage values.
	 *
	...
	/* No-one could re-use memory while we free all structures */
	{
		...
		for (i = 0; i < item_count; i++)
		{
			...
			if (query_entry)
			{
				...
				/*
				 * if no plans left - we could remove query_entry removing
				 * query could lead to store un-normalized query texts
				 * (normalized plan store only in post-parse steps), so we do
				 * not remove if really not need to. plans_count could be
				 * negative if query was removed during plan registration
				 */
		...
		/*
		 * if garbage collection is still needed - it means no execution plans
		 * left and pgqp_queries contains only items without plans - so make
		 * usual gc for queries (usefull if we do not store execution plans as
		 * we perform here usual GC)
		 */
 ...
 */
void
pgqp_entry_reset(Oid userid, Oid dbid, uint64 queryid)
{
	HASH_SEQ_STATUS hash_seq;
	pgqpEntry  *entry;
	...
	{
		/*
		 * Reset global statistics for pg_stat_query_plans since all entries
		 * are removed.
		 */
