 ...
 * the current jumble.
 */
void AppendJumble(pgqpJumbleState *jstate, const unsigned char *item,
                  Size size) {
 ...
 */
char *gen_normquery(pgqpJumbleState *jstate, const char *query, int query_loc,
                    int *query_len_p) {
  char *norm_query;
...
/*
 * comp_location: comparator for qsorting pgqpLocationLen structs by location
 */
...
#else
char *gen_normquery(JumbleState *jstate, const char *query, int query_loc,
                    int *query_len_p) {
  char *norm_query;
  int query_len = *query_len_p;
  int i, norm_query_buflen, /* Space allowed for norm_query */
...
StringInfo gen_normplan(char *execution_plan) {
  ...
  for (;;) {
...
#endif
    {
        ...
        if (need_replace(last_tok)) {
          ...
          tmp_str =
              strndup((char *)execution_plan + last_yylloc, yylloc - last_yylloc);
          appendStringInfoString(plan_out, tmp_str);
          free(tmp_str);
        }
    }
  ...
  PG_CATCH();
  {
	resetStringInfo(plan_out);
	appendStringInfoString(plan_out, execution_plan);
	FlushErrorState();
  }
  PG_END_TRY();
