...
StringInfo
_internal_parse(char *query, int *bind_prefix)
{
...
bool
do_parse(char *query, char *(*output_fnc) (const void *))
{
	int			bind_prefix = 1;
	...
	if (len > 0)
	{
		...
		/*
		 * dlen = pglz_decompress(compressed_dst, len, uncompressed_dst,
		 * plan_out->len, true);
		 *
		 * if (dlen < 0) printf("decompess failed \n"); else printf("%s \n",
		 * uncompressed_dst);
		 */
	}
